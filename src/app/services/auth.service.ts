...
import { AuthUser, LoginResponse } from '@shared/dto/auth.dto';
...
export class AuthService {
  ...
  async loginWithLocation(empCode: string, site: string, customLocation?: UserLocation): Promise<{ success: boolean; message: string; user?: AuthUser }> {
    try {
      ...
      const loginRequest: LoginRequestDto = {
        ...
        env: this.environmentService.getCurrentEnvironment().name
      };
      ...
      const apiUrl = this.environmentService.getProxyUrl()  + this.LOGIN_ENDPOINT;

      // Make API call
      const response = await firstValueFrom(
        this.http.post<any>(apiUrl, loginRequest)
      );

      console.log('API Response:', response);

      // Sementara return success buat testing proxy
      return { success: true, message: 'Login response received', };
  ...
  getSessionInfo(): {
    ...
    if (!currentUser) {
      return { isValid: false, expiresAt: null, minutesRemaining: 0, lastActivity: null };
    }
    const now = new Date();
    ...
    return { isValid: now < expiresAt, expiresAt, minutesRemaining, lastActivity: new Date(currentUser.lastActivityTime) };
  }
